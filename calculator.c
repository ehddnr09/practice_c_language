...
// "배운 것은 문자열을 비교할 때는 double quotes("")가 아닌 single quote('')를 써야 한다는 것을 알았다.
// scanf는 값 자체로 받아주기 때문에 scanf로 값의 타입을 확인하기 위해서는 다른 변수에 할당을 해서 비교를 해주면 된다는 것을 알았다.
// 여기서 1을 비교한 것은 claude를 통해서 알았는데 1로 비교했을 때 정수형 자료형인지 아닌지 확인할 수 있다.
// 다만 scanf로 받은 값을 변수에 할당한 값으로 비교해야 한다."